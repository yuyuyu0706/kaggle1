...
if __name__ == '__main__':
    ...
    # Count Encoding - mean
    df = do_count_mean(df, ['price'], 'user_id', 'uid_by_price_count_mean')
    df = do_count_mean(df, ['price'], 'uid_by_region_count_uniq', 'uid_by_price_loc_count_mean')
    df = do_count_mean(df, ['price'], 'uid_by_pcn_cn_count_uniq', 'uid_by_price_cat_count_mean')
    df = do_count_mean(df, ['price'], 'week', 'week_by_price_count_mean')
    df = do_count_mean(df, ['price'], 'image_top_1', 'it1_by_price_count_mean')
    df = do_count_mean(df, ['price'], 'item_seq_number', 'isn_by_price_count_mean')
    
    # Count Encoding - variance
    df = do_count_var(df, ['price'], 'user_id', 'uid_by_price_count_var')
    df = do_count_var(df, ['price'], 'uid_by_region_count_uniq', 'uid_by_price_loc_count_var')
    df = do_count_var(df, ['price'], 'uid_by_pcn_cn_count_uniq', 'uid_by_price_cat_count_var')
    df = do_count_var(df, ['price'], 'week', 'week_by_price_count_var')
    df = do_count_var(df, ['price'], 'image_top_1', 'it1_by_price_count_var')
    df = do_count_var(df, ['price'], 'item_seq_number', 'isn_by_price_count_var')
