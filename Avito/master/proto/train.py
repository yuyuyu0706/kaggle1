...
if __name__ == '__main__':
    ...
    df = do_count(df, ['user_id', 'avg_days_up_user'], 'uid_aduu_count')
    df = do_count(df, ['user_id', 'avg_times_up_user'], 'uid_atuu_count')
    df = do_count(df, ['user_id', 'n_user_items'], 'uid_nui_count')
    ...
    df = do_count_uniq(df, ['avg_days_up_user'], 'user_id', 'uid_by_aduu_count_uniq')
    df = do_count_uniq(df, ['avg_times_up_user'], 'user_id', 'uid_by_atuu_count_uniq')
    df = do_count_uniq(df, ['n_user_items'], 'user_id', 'uid_by_nui_count_uniq')
    ...
    df = do_count_mean(df, ['price'], 'avg_days_up_user', 'aduu_by_price_mean')
    df = do_count_mean(df, ['price'], 'avg_times_up_user', 'atuu_by_price_mean')
    df = do_count_mean(df, ['price'], 'n_user_items', 'nui_by_price_mean')
    ...
    df = do_count_var(df, ['price'], 'avg_days_up_user', 'aduu_by_price_var')
    df = do_count_var(df, ['price'], 'avg_times_up_user', 'atuu_by_price_var')
    df = do_count_var(df, ['price'], 'n_user_items', 'nui_by_price_var')
    ...
    df = do_count_std(df, ['price'], 'avg_days_up_user', 'aduu_by_price_std')
    df = do_count_std(df, ['price'], 'avg_times_up_user', 'atuu_by_price_std')
    df = do_count_std(df, ['price'], 'n_user_items', 'nui_by_price_std')
    ...
    # df.to_csv(DIR + "df_36.csv", index=True)
    ...
    # Save Data
    # sparse.save_npz('../input/train_vec_36.npz', ready_df)
    ...
    train_X, valid_X, train_y, valid_y = train_test_split(train_X,
                               train_y, test_size=0.10, shuffle=False, random_state=SEED)
