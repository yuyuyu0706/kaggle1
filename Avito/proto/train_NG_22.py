...
# def run_lgb(trn_X, trn_y, val_X, val_y, cat_vars):
    lg_params = {
    ...
    lg_trn = lgb.Dataset(trn_X, label=trn_y, categorical_feature=cat_vars)
    lg_val = lgb.Dataset(val_X, label=val_y, categorical_feature=cat_vars)
...
# def run_xgb(trn_X, trn_y, val_X, val_y, tfvocab):
    xg_params = {
        "max_depth": 15,           # [4, 6, 8, 12]
        "min_child_weight": 6,    # [4, 6, 8]
        "learning_rate": 0.019,     # [0.05, 0.075, 0.1, 0.2]
        "colsample_bytree": 0.8,
        "colsample_bylevel": 0.8,
        "reg_alpha": 0,
        }
...
if __name__ == '__main__':
    ...
    # df = pd.read_csv(DF_FILE, index_col=['item_id'])
    ...
    df['uid_by_price_mean'].fillna(-1, inplace=True)
    df['uid_by_price_loc_mean'].fillna(-1, inplace=True)
    df['uid_by_price_cat_mean'].fillna(-1, inplace=True)
    df['week_by_price_mean'].fillna(-1, inplace=True)
    df['it1_by_price_mean'].fillna(-1, inplace=True)
    df['isn_by_price_mean'].fillna(-1, inplace=True)
    ...
    df['uid_by_price_var'].fillna(-1, inplace=True)
    df['uid_by_price_loc_var'].fillna(-1, inplace=True)
    df['uid_by_price_cat_var'].fillna(-1, inplace=True)
    df['week_by_price_var'].fillna(-1, inplace=True)
    df['it1_by_price_var'].fillna(-1, inplace=True)
    df['isn_by_price_var'].fillna(-1, inplace=True)
    ...
    df['uid_by_price_var_ddof'].fillna(-1, inplace=True)
    df['uid_by_price_loc_var_ddof'].fillna(-1, inplace=True)
    df['uid_by_price_cat_var_ddof'].fillna(-1, inplace=True)
    df['week_by_price_var_ddof'].fillna(-1, inplace=True)
    df['it1_by_price_var_ddof'].fillna(-1, inplace=True)
    df['isn_by_price_var_ddof'].fillna(-1, inplace=True)
    ...
    df['uid_by_price_std'].fillna(-1, inplace=True)
    df['uid_by_price_loc_std'].fillna(-1, inplace=True)
    df['uid_by_price_cat_std'].fillna(-1, inplace=True)
    df['week_by_price_std'].fillna(-1, inplace=True)
    df['it1_by_price_std'].fillna(-1, inplace=True)
    ...
    df['uid_by_price_std_ddof'].fillna(-1, inplace=True)
    df['uid_by_price_loc_std_ddof'].fillna(-1, inplace=True)
    df['uid_by_price_cat_std_ddof'].fillna(-1, inplace=True)
    df['week_by_price_std_ddof'].fillna(-1, inplace=True)
    df['it1_by_price_std_ddof'].fillna(-1, inplace=True)
    ...
    df['title_desc_text_len_ratio'].fillna(-1, inplace=True)
    df['title_desc_words_len_ratio'].fillna(-1, inplace=True)
    df['title_desc_words_uniq_len_ratio'].fillna(-1, inplace=True)
    df['title_desc_wvu_ratio'].fillna(-1, inplace=True)
    df['title_desc_num_let_len_ratio'].fillna(-1, inplace=True)
    '''
    ...
    # exit()
    # df.to_csv(DIR + "df_35.csv", index=True)
    ...
    # Save Data
    # sparse.save_npz('../input/train_vec_35.npz', ready_df)
    
    # Load Data
    # ready_df = sparse.load_npz('../input/train_vec_34.npz')
    ...
    # model = run_lgb(train_X, train_y, valid_X, valid_y, cat_vars)
    ...
    # valid_X = pd.DataFrame(valid_X.todense())
    ...
    del train_X, train_y, valid_X, valid_y, cat_vars
    gc.collect()
