...
def read_csv(path):
    ...
    df = pd.read_csv(path, parse_dates=["activation_date"])
...
def read_csv_nrows(path, nrows):
    ...
    df = pd.read_csv(path, parse_dates=["activation_date"], nrows=nrows)
    df.drop_duplicates(subset='item_id', keep=False, inplace=True)
    df.set_index('item_id', inplace=True)
    logger.debug('exit')
    return df
...
def load_train_data(nrows=None):
    logger.debug('enter')
    if nrows is None:
        df1 = read_csv(TRAIN_DATA)
        df2 = read_csv(FEAT_DATA)
    else:
        df1 = read_csv_nrows(TRAIN_DATA, nrows)
        df2 = read_csv_nrows(FEAT_DATA, nrows)
    
    df1 = df1.merge(df2, on = ['user_id'], how = 'left')
